...
- Hermes PM runtime state under `agents/hermes/pm/runtime/` is operator-local. It is no longer a submodule: `agents/hermes/pm/.gitignore` ignores `runtime/` wholesale, `.gitmodules` is empty, and nothing under it is tracked. That directory holds `auth.json` and other credentials, so it must stay fully ignored — do not re-add it as a submodule.
