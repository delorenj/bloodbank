# Bloodbank tracing — operator workflow

This directory documents the **operator-facing tracing workflow** for Bloodbank
...
- `../../compose/nats/README.md` — subject topology used when walking
  a correlation chain.
...
|-----------------|----------------------------------------|-------------------------------------------------------------------------|
| `correlationid` | Envelope field on every envelope.   | Identifies the business transaction that ties related messages together. |
...
- Every event and command MUST carry `correlationid` / `correlation_id`.
- Every derived event or command MUST carry `causationid` /
...
- A trace backend (e.g. Jaeger, Tempo) becomes part of the `compose/`
  sandbox.
...
of the eventual tooling. `bb trace` is a stub.

Given a `correlationid` value `abc-123`, the operator expects to:

```text
# 1. Pull all events and commands in the correlation group.
nats stream view BLOODBANK_EVENTS   --subject 'event.>'    --filter 'data.correlationid == "abc-123"'
nats stream view BLOODBANK_COMMANDS --subject 'command.>'  --filter 'data.correlation_id == "abc-123"'
nats stream view BLOODBANK_COMMANDS --subject 'reply.>'    --filter 'data.correlation_id == "abc-123"'
...
## What `bb trace` will do (forward-looking)

```text
bb trace <correlationid>
...
Not implemented yet. The `cli/bb.py` stub has a placeholder.
