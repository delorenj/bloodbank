...
# commands (separate pubsub component bound to BLOODBANK_COMMANDS) is
# a future piece; the transport-level contract has to be right first.
#
# Usage:
#   bash ops/smoketest/smoketest-command.sh                       # fresh UUIDs
#   bash ops/smoketest/smoketest-command.sh --correlation-id MY   # deterministic
#
# Exit codes:
#   0 — PASS
...
if ! docker compose --project-name "${COMPOSE_PROJECT_NAME}" -f "${COMPOSE_FILE}" ps nats --format json >/dev/null 2>&1; then
  fail 1 "sandbox not reachable; run 'docker compose --project-name ${COMPOSE_PROJECT_NAME} -f compose/docker-compose.yml up -d nats nats-init'"
fi
...
# -----------------------------------------------------------------------------
# BLOODBANK_COMMANDS is a workqueue stream — consumers must use
