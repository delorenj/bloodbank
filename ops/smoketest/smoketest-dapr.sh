...
# This exercises the `bloodbank-pubsub` Dapr component end-to-end:
#
#   curl POST → daprd → pubsub.jetstream → NATS subject event.* → stream
#
# See ops/smoketest/README.md for scope. This is the Dapr-level
# companion to smoketest.sh (the pre-Dapr NATS-direct test).
#
# Usage:
#   bash ops/smoketest/smoketest-dapr.sh                         # fresh id
#   bash ops/smoketest/smoketest-dapr.sh --correlation-id MY_ID  # deterministic
#
# Exit codes:
#   0 — PASS
...
nats_run() {
  docker run --rm --network bloodbank-network natsio/nats-box:0.14.5 \
...
if ! docker compose --project-name "${COMPOSE_PROJECT_NAME}" --profile dapr-smoketest -f "${COMPOSE_FILE}" ps daprd-smoketest --format json >/dev/null 2>&1; then
  fail 1 "daprd-smoketest not running; bring up with: docker compose --project-name ${COMPOSE_PROJECT_NAME} --profile dapr-smoketest -f compose/docker-compose.yml up -d"
fi
