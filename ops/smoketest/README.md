...
the event backbone is wired correctly. Four complementary tests:
...
| `smoketest-dapr.sh` | Dapr `bloodbank-pubsub` component loads, publish HTTP API works, Dapr→NATS routing hits the expected subject | `nats` + `nats-init` + `dapr-placement` + `daprd-smoketest` |
...
| `smoketest-command.sh` | `BLOODBANK_COMMANDS` handles `command.*` → `reply.*` round-trip with correlation-ID preservation and workqueue drain-on-ack | `nats` + `nats-init` |

## Scope — `smoketest.sh` (NATS-direct)

Talks to NATS JetStream directly via the `nats` CLI in `nats-box`. It proves:

- `BLOODBANK_EVENTS` stream exists and accepts `event.smoketest.ping`.
...
- ADR-0001's topic-to-subject mapping holds: Dapr topic
  ...
  captured by the `BLOODBANK_EVENTS` stream's `event.>` binding.
...
- The `bloodbank-pubsub` component's subscribe path works end-to-end
  against `BLOODBANK_EVENTS`. Dapr creates a durable consumer
  (`bloodbank-pubsub` on the stream) and delivers matching events.
...
# Deterministic
bash bloodbank/ops/smoketest/smoketest-dapr.sh --correlation-id dapr-ci-$(date +%Y-%m-%d)
```
...
bash bloodbank/ops/smoketest/smoketest-dapr-subscribe.sh --correlation-id sub-ci-$(date +%Y-%m-%d)
```
...
1. Runs `nats-init` to ensure `BLOODBANK_EVENTS` exists (idempotent).
...
- `smoketest-command.sh` — exercises the command envelope and
  `BLOODBANK_COMMANDS` stream round-trip including `reply.*`
  correlation.
