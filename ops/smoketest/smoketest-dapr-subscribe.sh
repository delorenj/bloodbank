#!/usr/bin/env bash
#
# Bloodbank Dapr subscribe smoke test.
#
...
#
# Usage:
#   bash ops/smoketest/smoketest-dapr-subscribe.sh
#   bash ops/smoketest/smoketest-dapr-subscribe.sh --correlation-id MY_ID
#
# Exit codes:
#   0 — PASS
...
if ! docker compose --project-name "${COMPOSE_PROJECT_NAME}" --profile dapr-subscribe -f "${COMPOSE_FILE}" ps daprd-subscribe --format json >/dev/null 2>&1; then
  fail 1 "daprd-subscribe not running; bring up with: docker compose --project-name ${COMPOSE_PROJECT_NAME} --profile dapr-subscribe -f compose/docker-compose.yml up -d nats nats-init dapr-placement echo-sub daprd-subscribe"
fi
...
if [[ -z "${found}" ]]; then
  ...
  # on BLOODBANK_EVENTS). Dapr sets Nats-Msg-Id from the CloudEvent id,
