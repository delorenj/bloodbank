# Bloodbank platform bootstrap check

`check-platform.sh` is a **junior-friendly static validator** for Bloodbank
...
The script verifies the presence of the minimum scaffold artifacts --
...
The same list is cross-checked by `python cli/bb.py doctor`. If you
...
bash ops/bootstrap/check-platform.sh

# or from anywhere -- the script resolves its own location
bash /path/to/bloodbank/ops/bootstrap/check-platform.sh
```
...
2. Add the matching entry to `SCAFFOLD_MANIFEST` in `cli/bb.py` so
   `bb doctor` remains equivalent.
3. If the addition is expected before a scaffold wave completes, mark it
   ...
   in `bb doctor`; the shell script does not yet support `WARN` because
   its only job is strict CI gating.

Do not add checks that require Docker, Dapr, network I/O, or YAML/JSON
parsing to this script. That functionality belongs in `cli/bb.py`
or in later dedicated ops tooling.

## Relationship to `bb doctor`

| Aspect               | `check-platform.sh`                          | `bb doctor`                        |
|----------------------|-----------------------------------------------|---------------------------------------|
