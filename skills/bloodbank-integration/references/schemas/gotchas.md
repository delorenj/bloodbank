...
**Cause.** JSON Schema syntax is valid, but the schema's contract-facing fields do not match the Bloodbank v1 naming contract.

**Fix.** Check `$id`, `properties.type.const`, `properties.kind.const`, and the schema path under `schemas/bloodbank/v1/<domain>/`.
...
**Prevention.** Add a quick assertion in the producer (`assert envelope.type == "bloodbank.v1.agent.session.started"`) until you trust the schema.
...
**Symptom.** Producer publishes `bloodbank.v1.agent.session.started` with a v2 `schemaref`; consumer crashes parsing fields it expects from v1 (or vice versa).
...
**Symptom.** New `bloodbank/schemas/bloodbank/v1/<domain>/foo.bar.v1.json` exists but `mise run validate:schemas` ignores it.
...
**Fix.** Confirm the file lives under `schemas/bloodbank/v1/<domain>/` (not directly under `schemas/`). Tail `mise run validate:schemas` output for skipped-file warnings.
