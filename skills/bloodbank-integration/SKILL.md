...
├─ Yes → Dapr pub/sub. Subject = "bloodbank.evt.v1.<domain>.<entity>.<action>", pubsub component = bloodbank-pubsub.
...
- **`type` and NATS subject are bound.** The Dapr topic / NATS subject for an event is always the envelope's `subject`, derived from `type` by inserting `evt` after `bloodbank` (`bloodbank.v1.agent.tool.completed` → `bloodbank.evt.v1.agent.tool.completed`). Never publish to a subject that doesn't match the envelope.
- **Schema versioning is in the filename.** `schemas/bloodbank/v1/agent/session.started.v1.json` is v1; a breaking change becomes `.v2.json` with a new `dataschema` URI.
