...
Documentation-only scaffold for the replacement of the legacy **openclaw**
agent hooks. No executable migration code lives here yet.

## v2 component being replaced

`openclaw_bridge` (this repository, legacy). Today, openclaw agent hooks emit
...
- The v2 Bloodbank publisher role, which the architecture removes.
...
Same pattern as every adapter: translation only; envelope from Holyfields;
transport via Dapr.
...
1. **Schema mapping.** Which openclaw agent-hook events map to which
   ...
   (see HOLYF-2 (Holyfields contract tracker),
...
3. **Cutover coordination with hookd.** Openclaw and hookd share part of the
   inbound path in production. Their cutovers should land close together
   to avoid running two parallel envelope conventions for longer than
   necessary. Coordination point: metarepo plan.
