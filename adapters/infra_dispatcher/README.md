# infra_dispatcher adapter scaffold

Documentation-only scaffold for the replacement of the legacy
**infra_dispatcher**. No executable migration code lives here yet.

## v2 component being replaced

`event_producers/infra_dispatcher.py` (this repository, legacy). Today the
...
the central publish path; it is not acceptable in Bloodbank, where Bloodbank owns
...
- The adapter MUST NOT introduce a parallel publish transport; Dapr
  pub/sub `bloodbank-pubsub` is the only sanctioned route.

## Unknowns to resolve

1. **Repository home.** Does infra_dispatcher stay inside `bloodbank/`
   under `adapters/infra_dispatcher/`, or does it split into its own
   service? Arguments either way:
   - **Stay:** the dispatcher is thin; it does nothing broker-y once Bloodbank
...
This adapter is **blocked on HOLYF-2** (HOLYF-2 (Holyfields contract tracker)).
