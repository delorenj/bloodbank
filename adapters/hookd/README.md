# hookd adapter scaffold

Documentation-only scaffold for the replacement of the legacy **hookd**
bridge. No executable migration code lives here yet.

## v2 component being replaced

`hookd_bridge` (this repository, legacy). Today, hookd publishes via the
...
- Bloodbank's in-process publisher (a role the architecture explicitly
...
- Outbound traffic MUST route through Dapr `bloodbank-pubsub`.
...
1. **Schema mapping.** Which hookd event types map to which
   ...
   HOLYF-2 (Holyfields contract tracker), HOLYF-2).
